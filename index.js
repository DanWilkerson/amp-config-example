...
app.get('/gtm-analytics.config.json', (req, res) => {
    ...
    res.setHeader('Set-Cookie', cookieString)
        // cannot be a wildcard '*' when using Allow-Credentials
...
function getReferer(request) {
    let urlExtractor = /(https?:\/\/[^\/]+)\/?.*$/i;
    let refererMatch = request.headers.referer.match(urlExtractor);
    return refererMatch[refererMatch.length - 1];
}